...
BOT_SCOPED_KEYS = {"storage_channels", "upi_pool", "force_upi", "support_contact", "qr_unpaid_delete_minutes"}

def _scoped_cfg_key(key: str, bot_id_override=None) -> str:
    scoped = bot_id_override if bot_id_override is not None else BOT_ID
    if scoped and key in BOT_SCOPED_KEYS:
        key = f"{key}_bot_{scoped}"
    return key

def cfg(key, default=None, bot_id_override=None):
    # If BOT_ID is set (clone bot) OR a bot_id_override is passed, use bot-specific config keys
    key = _scoped_cfg_key(key, bot_id_override)
    doc = c_config.find_one({"key": key})
    return doc["value"] if doc and "value" in doc else default

def set_cfg(key, value, bot_id_override=None):
    # If BOT_ID is set (clone bot) OR a bot_id_override is passed, use bot-specific config keys
    key = _scoped_cfg_key(key, bot_id_override)
...
def start_purchase(ctx: CallbackContext, chat_id: int, uid: int, item_id: str):
    ...
    botid = self_bot_id(ctx.bot)
    uri = build_upi_uri(amt, f"order_bot_{botid}_uid_{uid}", chosen_upi)
    img = qr_url(uri)
    ...
    c_sessions.insert_one({
        ...
        "created_at": datetime.now(UTC), "admin_id": admin_id, "bot_id": botid,
...
# ---- Cross-bot helpers (main bot can deliver via clone bot token) ----
_SELF_BOT_ID_CACHE = None
_BOT_BY_ID_CACHE = {}   # bot_id -> telegram.Bot
_TOKEN_BY_ID_CACHE = {} # bot_id -> token

def self_bot_id(bot: Bot) -> int:
    """Return numeric bot id for this running process (clone uses BOT_ID env)."""
    global _SELF_BOT_ID_CACHE
    if BOT_ID:
        try:
            return int(BOT_ID)
        except Exception:
            pass
    if _SELF_BOT_ID_CACHE:
        return int(_SELF_BOT_ID_CACHE)
    try:
        _SELF_BOT_ID_CACHE = int(bot.get_me().id)
    except Exception:
        _SELF_BOT_ID_CACHE = 0
    return int(_SELF_BOT_ID_CACHE or 0)

def token_for_bot_id(bot_id: int, context_bot: Bot) -> str | None:
    """Resolve token for bot_id. Returns this process token for self, otherwise from accounts DB."""
    try:
        bot_id = int(bot_id)
    except Exception:
        return None
    ...
    # self
    if bot_id == self_bot_id(context_bot):
        return TOKEN

    if bot_id in _TOKEN_BY_ID_CACHE:
        return _TOKEN_BY_ID_CACHE[bot_id]

    doc = c_accounts.find_one({"clone_bot.id": int(bot_id)}, {"clone_bot.token": 1})
    token = ((doc or {}).get("clone_bot") or {}).get("token")
    if token:
        _TOKEN_BY_ID_CACHE[bot_id] = token
    return token

def bot_for_bot_id(bot_id: int, context_bot: Bot) -> Bot | None:
    """Return a telegram.Bot instance for bot_id (self uses context_bot)."""
    try:
        bot_id = int(bot_id)
    except Exception:
        return None
    ...
    if bot_id in _BOT_BY_ID_CACHE:
        return _BOT_BY_ID_CACHE[bot_id]

    token = token_for_bot_id(bot_id, context_bot)
    if not token:
        return None
    b = Bot(token)
    _BOT_BY_ID_CACHE[bot_id] = b
    return b

def _auto_delete_messages_via_token(ctx: CallbackContext):
    """JobQueue helper: delete messages using the *right* bot token."""
    data = ctx.job.context or {}
    token = data.get("token")
    chat_id = data.get("chat_id")
    mids = data.get("message_ids") or []
    if not token or not chat_id or not mids:
        return
    b = Bot(token)
    for mid in mids:
        try:
            b.delete_message(chat_id=chat_id, message_id=int(mid))
        except Exception:
            pass

def _robust_invite_link_bot(bot: Bot, ch_id: int) -> str | None:
    """Clone-safe copy of _robust_invite_link using a provided bot instance."""
    try:
    ...
    # Fallback: a normal invite link (no join request)
    try:
        inv = bot.create_chat_invite_link(
            chat_id=ch_id,
            name=f"auto_{int(time.time())}",
    ...
    # Fallback: public username link if available
    try:
...
def deliver_via_bot(bot: Bot, bot_id: int, uid: int, item_id: str, return_ids: bool = False, notify_on_fail: bool = False):
    """Deliver using a specific bot instance (main can deliver as clones)."""
    prod = c_products.find_one({"item_id": item_id})
    if not prod:
        try:
            bot.send_message(uid, "❌ Item missing.")
        except Exception:
            pass
        return [] if return_ids else None
    ...
    if "channel_id" in prod:
        ch_id = prod["channel_id"]
        link = _robust_invite_link_bot(bot, ch_id)
        if not link:
            if notify_on_fail:
                sup = cfg("support_contact", bot_id_override=bot_id)
                txt = "⚠️ This Channel is not available right now. Please contact support"
                txt += f" {sup}." if sup else "."
                try:
    ...
    storage_list = cfg("storage_channels", bot_id_override=bot_id)
    if not storage_list:
        log.error(f"No storage channels configured for bot {bot_id} item {item_id}")
        if notify_on_fail:
            sup = cfg("support_contact", bot_id_override=bot_id)
            txt = "⚠️ Storage not configured. Please contact support"
            txt += f" {sup}." if sup else "."
    ...
    main_storage = int(storage_list[0])

    def _copy_variant(from_chat_id: int, message_id: int) -> int | None:
        try:
            m = bot.copy_message(chat_id=uid, from_chat_id=int(from_chat_id), message_id=int(message_id))
    ...
    for f in prod.get("files", []):
        ...
        # Prefer main storage variant first
        main_variant = None
        if primary[0] == main_storage:
            main_variant = primary
        else:
        ...
        if not delivered:
            log.error(f"All storage variants failed for a file of item {item_id} (bot {bot_id})")

    try:
        bot.send_message(uid, "⚠️ Files auto-delete here in 30 minutes. Save now.")
    except Exception:
        pass
...
def on_channel_post(update: Update, context: CallbackContext):
    # NOTE:
    # Only the MAIN bot is admin in PAYMENT_NOTIF_CHANNEL_ID, so only MAIN will receive these updates.
    # When a payment matches a session created by a CLONE, the MAIN bot delivers using the clone's token
    # (same concept as "bot #1 listens, bot #2 sends via raw Bot API").
    ...
    for s in matches:
        # Backward compat: older sessions won't have bot_id; assume current process bot id.
        target_bot_id = int(s.get("bot_id") or self_bot_id(context.bot))
        bot = bot_for_bot_id(target_bot_id, context.bot)
        token = token_for_bot_id(target_bot_id, context.bot)

        if not bot or not token:
            log.warning(f"Cannot resolve bot/token for bot_id={target_bot_id}. Skipping delivery.")
            continue

        # Delete unpaid QR (must be deleted by the bot that sent it)
        qr_mid = s.get("qr_message_id")
        if qr_mid:
            try:
                bot.delete_message(chat_id=s["chat_id"], message_id=int(qr_mid))
            except Exception as e:
                log.debug(f"Delete QR failed (bot {target_bot_id}): {e}")

        confirm_msg_id = None
        try:
            confirm_msg = bot.send_message(s["chat_id"], "✅ Payment received. Delivering your item…")
            confirm_msg_id = confirm_msg.message_id
        except Exception as e:
            log.warning(f"Notify user fail (bot {target_bot_id}): {e}")

        ids_to_delete = []
        if confirm_msg_id:
            ids_to_delete.append(confirm_msg_id)

        # Deliver using the correct bot
        deliver_ids = deliver_via_bot(bot, target_bot_id, s["user_id"], s["item_id"], return_ids=True, notify_on_fail=False)
        ...
        # For channel products, keep the same order/receipt behavior but send via the correct bot
        if "channel_id" in prod:
            try:
                c_orders.update_one(
            ...
            try:
                ...
                r = bot.send_message(s["user_id"], receipt_text, parse_mode=ParseMode.MARKDOWN)
            ...
            if not deliver_ids:
                sup = cfg("support_contact", bot_id_override=target_bot_id)
                txt = "⚠️ This Channel is not available right now. Please contact support"
                txt += f" {sup}." if sup else "."
                try:
                    bot.send_message(s["user_id"], txt)
                except Exception as e:
        ...
        # Auto-delete link / delivered files / "payment received" message
        if ids_to_delete:
            context.job_queue.run_once(
                _auto_delete_messages_via_token,
                timedelta(minutes=DELETE_AFTER_MINUTES),
                context={"token": token, "chat_id": s["chat_id"], "message_ids": ids_to_delete},
